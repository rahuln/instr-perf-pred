...
def parse_args():
    ...
    parser.add_argument("--compute_per_instance_metrics", action="store_true", help="Compute metrics on every evaluation instance.")
    parser.add_argument("--outfile", type=str, default="predict_results.json", help="name of output file.")
...
if __name__ == "__main__":
    ...
    category_metrics = [
        ...
        ("Word Analogy", "rougeL"),
        ("Overlap Extraction", "exact_match"),
        ("Keyword Tagging", "exact_match"),
        ("Question Rewriting", "exact_match"),
        ("Title Generation", "exact_match"),
        ("Data to Text", "exact_match"),
        ("Grammar Error Correction", "exact_match"),
    ]
    ...
    if args.compute_per_category_metrics:
        task_num_to_files = {}
        for file in glob.glob(os.path.join("data/eval/superni/tasks/", "*.json")):
    ...
    if args.compute_per_task_metrics:
        ...
        results.update(results_by_task)
        print()

    if args.compute_per_instance_metrics:
        print("======== Metrics per task ========")
        instance_ids = [e["Instance"]["id"] for e in examples]
        results_by_instance = compute_grouped_metrics(predictions, references, instance_ids, xlingual=args.track=="xlingual")
        results.update(results_by_instance)
        print()

    results = {f'predict_{key}' : results[key] for key in sorted(list(results.keys()))}
    with open(os.path.join(os.path.dirname(args.predictions), args.outfile), 'w') as f:
